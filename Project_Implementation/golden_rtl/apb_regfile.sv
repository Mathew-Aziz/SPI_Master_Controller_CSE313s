...
module apb_regfile (
  ...
  wire [31:0] ctrl_word = {
    24'b0, ctrl_width, ctrl_loopback, ctrl_lsb_first, ctrl_mode, ctrl_mstr, ctrl_en
  };
  ...
  always @(posedge PCLK or negedge PRESETn) begin
    if (!PRESETn) begin
      ...
      begin : int_stat_update
        ...
        if (tx_push_dropped) next_stat[IRQ_TX_OVF] = 1'b1;
        if (rx_push_valid && rx_full_w) next_stat[IRQ_RX_OVF] = 1'b1;
        if (rx_push_valid && !rx_full_w && (rx_count == FIFO_DEPTH - 1))
          next_stat[IRQ_RX_FULL] = 1'b1;
        if (tx_pop && (tx_count == 1)) next_stat[IRQ_TX_EMPTY] = 1'b1;
        if (transfer_done_pulse) next_stat[IRQ_TRANSFER_DONE] = 1'b1;
  ...
  // BUG NOTE: R3 Violation in golden RTL 

  // Requirement R3 (Section 9): "CTRL.EN=0 ... SS_n forced high regardless of SS_CTRL"
  // The provided golden RTL implements: assign SS_n = ~ss_en | ss_val;   // Ignores ctrl_en
  //
  // Expected behaviour when EN=0: SS_n = 4'hF (all bits high)
  // Observed behaviour when EN=0 and SS_CTRL=0x0000_0001: SS_n = 4'hE (bit0 low)
  //
  // This test (TC-8) correctly detects the violation. The DUT must be fixed to:
  //     assign SS_n = ctrl_en ? (~ss_en | ss_val) : 4'hF;

  // Requirement R3 says: when the global enable CTRL.EN = 0,
  //    every SS_n output must be forced high (4'hF) regardless of what is programmed in SS_CTRL. 
  //   This ensures no slave is accidentally selected when the SPI master is disabled,
  //    preventing bus contention or spurious transfers.

  // The bug you found is that the golden RTL ignores CTRL.EN and drives SS_n purely based on SS_CTRL. 
  //   Therefore, when EN = 0, SS_n[0] can remain low if SS_EN[0]=1 and SS_VAL[0]=0, violating the specification.
